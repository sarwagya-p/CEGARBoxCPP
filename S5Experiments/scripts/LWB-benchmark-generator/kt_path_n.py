...
def kt_path_n(n):
    return str(kt_path_n_f(n))[1:-1]

def kt_path_n_f(n):
    d1 = []
    for i in [1, path_el(1, n+1), 3, 5]: d1.append(Box(p(10+i)))
    d2 = []
    for i in range(1, n+1+1):
        for j in range(1, 6+1):
            d2.append(left_to_right(i, j, n+1) |OR| right_to_left(i, j, n+1))
    d3 = []
    for i in [2, 4, path_el(n+1, n+1), 6]: d3.append(mdia(n+1, Not(p(10*(n+1) + i))))
...
def left_to_right(level, k, n):
    ...
    if (k%2 == 0) and (k == path_el(level, n)) and (level == n//2): return FALSE
    if (k%2 == 0): return lists2fml(level, k, [x])
    if k == path_el(level, n):
        if level == n//2: return lists2fml(level, k, [1, 3, 5])
        else: return lists2fml(level, k, [1, 3, x, 5])
...
def lists2fml(level, k, s):
    d = []
    for x in s:
        d.append(mdia(level, Not(p(10*level+k)) |AND| Box(p(10 * (level+1) + x))))
    return list2disj(d)

def lists2fml_back(level, k, s):
    d = []
    for x in s:
        d.append(mdia(level, Not(p(10*level+x)) |AND| Box(p(10 * (level+1) + k))))
    return list2disj(d)